...
- **Implementation with agreed small scope** (existing design doc or trivial change) → a **builder**, picked by DOMAIN — **you must do this mapping yourself**: sub-agents can't dispatch sub-agents.
  ...
  - **master-data & time-series binding** (join CSV/API master data by IFC GlobalId, DataBus subscriptions, live value → material/color/label updates, element pick → property panel; **connecting a real MQTT broker / plant telemetry** = the **already-built** MQTT→WS bridge `tools/bridge/mqtt_ws.js` + its `mqtt_map.json` topic→tag map behind the `sourceUrl` seam — configure and run it, it is NOT a new ingress to design or build) → `xenodot:twin-data-binder`
...
1. Confirm the RIGHT gates ran — a twin-data-binder slice that only ran render health is not verified; re-dispatch to run the twin gate, don't wave it through.
