...
/** The xenodot spine ALWAYS loads — ONE plugin that carries both the engine-generic capabilities
 * and the folded-in digital-twin domain (twin agents/skills). The OPTIONAL Codex reviewer is
 * appended only when enabled AND vendored on disk. Gating is array inclusion (the SDK has no
 * per-plugin enable flag, and `plugins` only accepts `{ type: "local" }`).
