...
| 4   | Benched LOD/visibility-range recipe                             | [2026-07-09-vis-range-recipe-plan.md](2026-07-09-vis-range-recipe-plan.md) | ✅ Built on `feat/vis-range-recipe` — SCOPED WIN (unique-city aerial cpu −32%, no-op on single buildings/instanced; defaults kept, opt-in), pending merge |
...
| 6   | Web/Grafana embed story                          | [2026-07-09-web-embed-plan.md](2026-07-09-web-embed-plan.md)               | ✅ Built on `feat/web-embed` — seat-measured (Chrome 12/12, floor PASS ~4×). Outcome: FULL GRAFANA RECIPE (no-threads embeds at 120 fps in a real Grafana iframe; threads DOA there; no-threads costs 0 fps); scale ceiling ~17 fps at 28.6k unique meshes, Safari unverified. Shipped `serve_coi.py` + presets + docs; pending merge                                                                                                                                                                                                |
| 7   | `twin-ship` packaging skill                      | [2026-07-09-twin-ship-plan.md](2026-07-09-twin-ship-plan.md)               | ✅ Built on `feat/twin-ship` (3 phases) — viewer made export-safe (`84106ba`: buffer-load, exe-adjacent cfg, `--quit-after`), `twin_ship.sh` 5 stages + data-beside-build contract (`8d8d7c3`), skill + docs + seat-validated artifact (clean-stranger unzip → README-only run, windowed boot on Metal, data-swap without re-export, Linux SKIP loud); pending merge                                                                                                                                                                 |
| 8   | Second demo asset (plant/factory-flavored model) | [2026-07-09-plant-demo-asset-plan.md](2026-07-09-plant-demo-asset-plan.md) | ✅ Built on `feat/plant-demo-asset` (3 phases) — **TRACK B (synthetic)**: no public model qualified (spike verdicts), so a self-owned `gen_plant_ifc.py` IFC4 tank-farm/pump-skid ships labeled "synthetic demonstration model"; vendored default binds 8 tags across pump/tank/valve, clean-stranger green (join 18/18). Honest optimizer read: **zero instancing at any scale** (unique per-element geometry) — flavor/data-binding showcase, not an instancing one; deterministic recording + shot-list note added; pending merge |
