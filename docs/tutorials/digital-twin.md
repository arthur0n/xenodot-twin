...
**Wrinkle worth knowing:** on a fresh clone `npm run validate` can fail on a stale generated
index — e.g. `plugin/library/verdicts/index.md: stale`. The error prints the exact fix —
`npm run check:library -- --write` — after which `validate` exits 0. (It's a one-character
escaping drift in a generated index; regenerating rewrites it.)
