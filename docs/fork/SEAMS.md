...
| `ui/client/features/assets/get-assets.js`                                                          | The rendered "Place" dropdown option `Game (assets/)` → `Project (assets/)` (+ its adjacent JSDoc `game's own assets/` / `game tree` → `project`). The `place` value token `"game"` stays (code identifier — the server API contract). **Upstream candidate — neutral wording, offer as PR.**                                                                                                                                                                                                                                                                                 | The option label is an inline `el("option", …, "Game (assets/)")` string the user sees in the assets modal; embedded, not appendable.                                |
