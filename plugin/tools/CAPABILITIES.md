...
| `check_scene_export_assigned.gd` | The "third data-driven half" — asserts each named `@export` is wired **non-null on the live node** in the shipped `.tscn` (authored + read is not enough; an unwired export is a dead feature behind a green gate). Enrolled scenes come from the game's `design/export-wiring.tsv` (`<scene.tscn> <NodePath> <field>` per line), which `check_export_assigned` (`tools/lib/checks.sh`, on the `validate.sh` floor) reads — no file = SKIP.                                                      | `$GODOT --headless --path . --script tools/check_scene_export_assigned.gd -- <scene.tscn> <NodePath> <field> [<NodePath> <field> ...]`                                                                                                                                                                                                                           | Headless OK. Contract + rationale: skill `godot-data-driven-composition`                                                                                                                                                                                                   |
...
## Digital-twin viewer tools

The digital-twin domain folded into the one xenodot plugin, so these materialize into a viewer
project's `tools/` in the SAME pass as the base tools above (`validate.sh`, `lib/checks.sh`,
...
`plugin/library/findings/twin-ship-2026-07-10.md`.
...
scenes; defaults kept, still opt-in): `plugin/library/findings/twin-vis-range-recipe-2026-07-09.md`.
...
`plugin/library/findings/twin-vis-fade-2026-07-10.md`.
...
`plugin/library/findings/twin-occluder-recipe-2026-07-10.md`.
...
`twin_build.sh`. Worked example: `plugin/examples/bench_sweep.vis-fade.example.json`; its
`scene_in` is the many-unique-mesh scale/bench city built by `plugin/examples/gen_city.gd`
...
the two contracts: `plugin/skills/twin-analyze/references/{bundle-schema,report-format}.md`.
Seat-proven worked example: `plugin/library/findings/twin-analyze-2026-07-10.md`.
...
`plugin/examples/mqtt_map.example.json`) maps topics→tags: `topic` (exact or `+`/`#`
...
`plugin/examples/mqtt_map.example.json` maps — with smooth animated values (reusing
...
`plugin/examples/export_presets.web-nothreads.cfg` (the embed-anywhere / Grafana build) +
...
`plugin/library/findings/twin-web-ceiling-2026-07-10.md`. Recipe + embed snippet: skill
...
These resolve from the one xenodot plugin (the twin domain folded in) / the project `tools/`:
...
- `tools/lib/checks.sh`, `validate.sh`, `verify_scene.gd`, `verify_render.gd` — the base
  gate scripts `verify_twin.sh` composes or defers to (materialized from the plugin).
