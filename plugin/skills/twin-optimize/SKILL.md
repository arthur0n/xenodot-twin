...
- **Declarative matrix (JSON).** Names the `scene_in`, the `configs` (each a name → optimizer
  ...
  `plugin/examples/bench_sweep.vis-fade.example.json` — its `scene_in` is the many-unique-mesh
  city built by `plugin/examples/gen_city.gd` (the scale/bench demo scene; run it to regenerate
