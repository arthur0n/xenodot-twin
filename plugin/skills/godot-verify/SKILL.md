...
universal.** A scene that renders through a **SubViewport** (e.g. a low-res or offscreen
render rig) needs Layers 3/4 to sample that SubViewport texture. A scene with no such rig
renders the 3D straight to the **root** viewport, so Layers 3/4 already sample root — the
usual case for a digital-twin viewer. Never assume a SubViewport rig exists; scope
...
key event and step frames through the real input path (an `extends SceneTree` input-sim
harness), THEN capture.
...
(an `extends SceneTree` input-sim harness) AND (b) **capturing the relevant viewport and ACTUALLY
...
5. Interactive / on-screen change (UI panel, overlay, property inspector, element pick): real-input-path sim (an `extends SceneTree` harness) AND a captured-and-INSPECTED frame (Layer 5 root viewport for CanvasLayer UI) — not a "human F5" punt. A captured anomaly is a finding, not waved off as "expected".
...
| Render OK but scene behaviorally broken (elements inert/floating, nothing binds, an overlay never updates)                                           | Layer 3 only proves something rendered, not that the scene _behaves_. Asserting the scene actually behaves (expected node/element counts, bound values move, an interaction fires) is **godot-runtime-smoke**'s job — a headless `smoke_*.gd` that boots the scene, steps frames, and asserts invariants; for the twin's join / binding / playback layers use **twin-verify**.                                                                                                                             |
...
| Reported a UI / visual / toggle feature "verified" (or "human F5 needed") without a capture                                                          | If it is on-screen and capturable, capture + INSPECT it yourself (Layer 3/4/5) and drive the real input path (an `extends SceneTree` harness). "human F5" is only for the genuinely uncapturable. Never wave off a visible anomaly in a capture as "expected" without a stated reason.                                                                                                                                                                                                                     |
