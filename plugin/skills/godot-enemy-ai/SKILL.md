...
- A `NavigationRegion3D` in the level whose `NavigationMesh` covers the walkable floor. Without a usable mesh the agent finds no path and the enemy stands still. **On Godot 4.6 the EDITOR "Bake NavigationMesh" silently ships an EMPTY mesh** — runtime-bake it in the level `_ready()` instead. This and every other 4.6 nav gotcha (nav-map sync, target-snap onto the mesh, entity-velocity ordering vs a base-class `_physics_process`) live in **`godot-navmesh-pathing-4-6`**, the integration layer UNDER this skill — read it before assuming a baked mesh.
- `godot-stealth-perception` — if the enemy needs a vision **cone** + **hearing** + an Alert/Search FSM (a stealth guard) rather than the cheap distance + single `RayCast3D` LOS this skill ships. Pick one perception model deliberately; don't run both.
- `godot-enemy-ai-headless-smoke` — read BEFORE writing any headless test of this AI. Real pathing / vision-cone / hearing are NOT assertable headless (no `RenderingDevice`, no bake, no synchronous `Area3D` overlap); that skill is the can/can't-test split (assert FSM logic headless, leave path/see/hear to a windowed F5).
...
| Enemies frozen on a fresh Godot 4.6 level — killable but never move, after an editor bake          | The 4.6 editor "Bake NavigationMesh" silently ships an EMPTY navmesh (threaded bake not flushed to the `.tscn`), so every `NavigationAgent3D` finds no path; hit/death still run on collision layers, hence killable. Runtime-bake in the level `_ready()` via `NavigationRegion3D.bake_navigation_mesh()` — the source of truth. Full 4.6 nav landmine set: `godot-navmesh-pathing-4-6`.                                                                                                                                                                                                                                         |
