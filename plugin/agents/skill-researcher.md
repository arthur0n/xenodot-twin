...
6. **On adopt — rewrite, never copy.** Create `.claude/skills/godot-<name>/SKILL.md` in this project's skill template:
   - Frontmatter: `name: godot-<system>[-<qualifier>]` — **system first, optional qualifier last** (e.g. `godot-mesh-import-hd`, with the "hd" qualifier at the END, not the front), and **keep the engine version OUT of the name** — pin it in frontmatter / the `## Requirements` section, never the name (a version like `4-6` baked into a name outlives the version it pins). Then `description:` stating what it does AND the concrete trigger phrases/situations for when to use it. (The `gen-skill-scope` gate warns on version-in-name; qualifier order is yours to get right — existing inconsistent names stay, this is for NEW skills.)
