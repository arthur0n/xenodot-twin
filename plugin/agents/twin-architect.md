...
2. **Data-binding map** — which live tags bind to which elements, joined by **IFC GlobalId** (the GLB node names carry them — skill `twin-import`). The map is data (`design/binding-map.json` or a table in the doc), never hard-coded lookups. State the fixture: the seeded simulator (skill `twin-bind-data`) is the default until a real source exists. A real MQTT broker is **not** a new ingress to design — the MQTT→WS bridge (`tools/bridge/mqtt_ws.js`, skill `twin-bind-data`) **already exists** behind the `sourceUrl` seam; specify its `mqtt_map.json` topic→tag mapping and point `sourceUrl` at it, rather than designing a second data path.
